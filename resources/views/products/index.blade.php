...
<div class="mb-4 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
    <form action="{{ route('products.index') }}" method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <select name="category_id" class="h-11 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 sm:w-48">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $products->firstItem() + $i }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div><p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $product->name }}</p><p class="text-theme-xs text-gray-500">SKU: {{ $product->sku }}</p></div>
                            </div>
                        </td>
                        <td class="px-4 py-3"><span class="rounded-full bg-brand-50 px-2.5 py-1 text-theme-xs font-medium text-brand-500 dark:bg-brand-500/10">{{ $product->category->name ?? '-' }}</span></td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-800 dark:text-white/90">Rp {{ number_format($product->price, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            @if($product->stock <= 5)
                            <span class="rounded-full bg-error-50 px-2.5 py-1 text-theme-xs font-medium text-error-500 dark:bg-error-500/10">{{ $product->stock }}</span>
                            @else
                            <span class="rounded-full bg-success-50 px-2.5 py-1 text-theme-xs font-medium text-success-500 dark:bg-success-500/10">{{ $product->stock }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('products.destroy', $product) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">@csrf @method('DELETE')
